...
function AppContent() {
  ...
  // Load user's saved podcasts when user changes
  useEffect(() => {
    if (user) {
      // Sort podcasts by timestamp in descending order (newest first)
      const sortedPodcasts = [...(user.savedPodcasts || [])].sort((a, b) => {
        return (b.timestamp || 0) - (a.timestamp || 0);
      });
      setMyPodcasts(sortedPodcasts);
    } else {
      setMyPodcasts([]);
    }
  }, [user]);
  ...
  const handlePodcastGenerated = (newPodcast) => {
    // Add timestamp to podcast
    const podcastWithTimestamp = {
      ...newPodcast,
      timestamp: Date.now()
    };
    setPodcasts(prev => [...prev, podcastWithTimestamp]);
    setCurrentPodcast(podcastWithTimestamp);
    setShowGenerator(false);
  }

  const handleAddToMyPodcasts = (podcast) => {
    if (podcast === null) {
      // Remove the last added podcast
      setMyPodcasts(prev => prev.slice(0, -1));
    } else {
      // Add new podcast if it doesn't exist
      if (!myPodcasts.some(p => p.id === podcast.id)) {
        // Add timestamp if not exists
        const podcastWithTimestamp = podcast.timestamp ? podcast : {
          ...podcast,
          timestamp: Date.now()
        };
        // Add to the beginning of the list (newest first)
        setMyPodcasts(prev => [podcastWithTimestamp, ...prev]);
      }
    }
  }
